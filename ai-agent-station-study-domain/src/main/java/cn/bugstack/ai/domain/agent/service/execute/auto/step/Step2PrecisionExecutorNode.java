...
public class Step2PrecisionExecutorNode extends AbstractExecuteSupport{
    ...
    protected String doApply(ExecuteCommandEntity requestParameter, DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext) throws Exception {
        ...
        if (aiAgentClientFlowConfigVO == null) {
            throw new IllegalStateException("缺少 PRECISION_EXECUTOR_CLIENT 流程配置");
        }

        String executionPrompt = String.format(aiAgentClientFlowConfigVO.getStepPrompt(), requestParameter.getMessage(), analysisResult);
        executionPrompt = promptContextBuilder.appendExecutionContext(executionPrompt, requestParameter);
        ...
        if (executionResult == null || executionResult.isBlank()) {
            throw new IllegalStateException("任务执行模型返回空结果");
        }
        ...
        agentRunService.checkpoint(requestParameter, "EXECUTION", dynamicContext.getStep(),
                stepSummary, "进入质量监督");
