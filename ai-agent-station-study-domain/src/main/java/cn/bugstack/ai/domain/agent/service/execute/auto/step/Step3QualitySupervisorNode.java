...
public class Step3QualitySupervisorNode extends AbstractExecuteSupport {
    ...
    protected String doApply(ExecuteCommandEntity requestParameter, DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext) throws Exception {
        ...
        if (executionResult == null || executionResult.trim().isEmpty()) {
            throw new IllegalStateException("执行结果为空，不能进入质量监督");
        }
        ...
        if (aiAgentClientFlowConfigVO == null) {
            throw new IllegalStateException("缺少 QUALITY_SUPERVISOR_CLIENT 流程配置");
        }
        ...
        supervisionPrompt = promptContextBuilder.appendSupervisionContract(supervisionPrompt, requestParameter);
        ...
        if (supervisionResult == null || supervisionResult.isBlank()) {
            throw new IllegalStateException("质量监督模型返回空结果");
        }
        parseSupervisionResult(dynamicContext, supervisionResult, requestParameter.getSessionId());

        AgentSupervisionDecision decision = decisionParser.parseSupervision(supervisionResult);
        dynamicContext.setValue("supervisionDecision", decision);
        
        // 将监督结果保存到动态上下文中
        dynamicContext.setValue("supervisionResult", supervisionResult);
        
        // 根据监督结果决定是否需要重新执行
        if (decision.getVerdict() == AgentSupervisionDecision.Verdict.FAIL) {
            log.info("❌ 质量检查未通过，需要重新执行");
            dynamicContext.setCurrentTask(decision.getNextAction() == null
                    ? "根据质量监督的建议重新执行任务" : decision.getNextAction());
        } else if (decision.getVerdict() == AgentSupervisionDecision.Verdict.OPTIMIZE) {
            log.info("🔧 质量检查建议优化，继续改进");
            dynamicContext.setCurrentTask(decision.getNextAction() == null
                    ? "根据质量监督的建议优化执行结果" : decision.getNextAction());
        } else {
            log.info("✅ 质量检查通过");
        ...
        agentRunService.checkpoint(requestParameter, "SUPERVISION", dynamicContext.getStep(),
                stepSummary, dynamicContext.isCompleted() ? "生成最终总结" : dynamicContext.getCurrentTask());
