...
public class Step1McpToolsAnalysisNode extends AbstractExecuteSupport {
    ...
    protected String doApply(ExecuteCommandEntity requestParameter, DefaultFlowAgentExecuteStrategyFactory.DynamicContext dynamicContext) throws Exception {
        ...
        if (aiAgentClientFlowConfigVO == null) {
            throw new IllegalStateException("缺少 TOOL_MCP_CLIENT 流程配置");
        }
        ...
        mcpAnalysisPrompt = promptContextBuilder.appendExecutionContext(mcpAnalysisPrompt, requestParameter);
        ...
        if (mcpToolsAnalysis == null || mcpToolsAnalysis.isBlank()) {
            throw new IllegalStateException("MCP 工具分析返回空结果");
        }
        ...
        agentRunService.checkpoint(requestParameter, "FLOW_TOOL_ANALYSIS", dynamicContext.getStep(),
                mcpToolsAnalysis, "生成结构化执行计划");
