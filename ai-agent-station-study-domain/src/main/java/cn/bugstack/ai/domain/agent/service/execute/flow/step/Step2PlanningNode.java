...
public class Step2PlanningNode extends AbstractExecuteSupport {
    ...
    protected String doApply(ExecuteCommandEntity requestParameter, DefaultFlowAgentExecuteStrategyFactory.DynamicContext dynamicContext) throws Exception {
        ...
        if (aiAgentClientFlowConfigVO == null) {
            throw new IllegalStateException("缺少 PLANNING_CLIENT 流程配置");
        }
        ...
        String planningPrompt = buildStructuredPlanningPrompt(userRequest, mcpToolsAnalysis, requestParameter);
        ...
        if (planningResult == null || planningResult.isBlank()) {
            throw new IllegalStateException("执行规划模型返回空结果");
        }
        ...
        agentRunService.checkpoint(requestParameter, "FLOW_PLANNING", dynamicContext.getStep(),
                planningResult, "解析并执行计划");
     ...
     * 构建结构化的规划提示词
     */
    private String buildStructuredPlanningPrompt(String userRequest, String mcpToolsAnalysis, ExecuteCommandEntity command) {
        ...
        // 动态获取实际的MCP工具信息
        String actualToolsInfo = "服务端授权工具模式: " + command.getAllowedTools()
                + "\n高风险工具只有出现在 approvedToolNames 中才可执行。";
        ...
        return promptContextBuilder.appendExecutionContext(prompt.toString(), command);
    }
