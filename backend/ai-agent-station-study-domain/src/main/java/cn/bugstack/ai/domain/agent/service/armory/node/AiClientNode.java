...
public class AiClientNode extends AbstractArmorySupport {
    ...
    protected String doApply(ArmoryCommandEntity requestParameter, DefaultArmoryStrategyFactory.DynamicContext dynamicContext) throws Exception {
        ...
        for (AiClientVO aiClientVO : aiClientList) {
            ...
            // 5. 构建对话客户端
            ToolCallback[] governedCallbacks = toolGovernanceService.wrap(
                    new SyncMcpToolCallbackProvider(mcpSyncClients.toArray(new McpSyncClient[]{})).getToolCallbacks());

            ChatClient chatClient = ChatClient.builder(chatModel)
                    .defaultSystem(defaultSystem.toString())
                    .defaultToolCallbacks(governedCallbacks)
