...
import cn.bugstack.ai.domain.agent.service.runtime.AgentDecisionParser;
...
public class Step1AnalyzerNode extends AbstractExecuteSupport {
    ...
    protected String doApply(ExecuteCommandEntity requestParameter, DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext) throws Exception {
        ...
        if (aiAgentClientFlowConfigVO == null) {
            throw new IllegalStateException("缺少 TASK_ANALYZER_CLIENT 流程配置");
        }
        ...
        analysisPrompt = promptContextBuilder.appendAnalysisContract(analysisPrompt, requestParameter);

        ChatClient chatClient = getChatClientByClientId(aiAgentClientFlowConfigVO.getClientId());

        String analysisResult = chatClient
                .prompt(analysisPrompt)
                .toolContext(toolContext(requestParameter))
                .advisors(a -> a
        ...
        if (analysisResult == null || analysisResult.isBlank()) {
            throw new IllegalStateException("任务分析模型返回空结果");
        }
        sendSseResult(dynamicContext, AutoAgentExecuteResultEntity.createAnalysisResult(
                dynamicContext.getStep(), analysisResult, requestParameter.getSessionId()));
        parseAnalysisResult(dynamicContext, analysisResult, requestParameter.getSessionId());

        AgentAnalysisDecision decision = decisionParser.parseAnalysis(analysisResult);
        dynamicContext.setValue("analysisDecision", decision);
        // 将分析结果保存到动态上下文中，供下一步使用
        dynamicContext.setValue("analysisResult", analysisResult);

        agentRunService.checkpoint(requestParameter, "ANALYSIS", dynamicContext.getStep(),
                analysisResult, decision.getNextAction());

        if (decision.getStatus() == AgentAnalysisDecision.Status.WAIT_USER_INPUT) {
            String question = decision.getClarificationQuestion();
            if (question == null || question.isBlank()) {
                question = "继续执行前需要补充: " + decision.getMissingFields();
            }
            ResponseBodyEmitter emitter = dynamicContext.getValue("emitter");
            agentRunService.markWaiting(requestParameter, emitter, question, decision.getSummary());
            dynamicContext.setWaitingUserInput(true);
            dynamicContext.setCompleted(true);
            return router(requestParameter, dynamicContext);
        }

        // 检查是否已完成
        if (decision.getStatus() == AgentAnalysisDecision.Status.COMPLETED || decision.getProgress() >= 100) {
            dynamicContext.setCompleted(true);
            log.info("✅ 任务分析显示已完成！");
            return router(requestParameter, dynamicContext);
        }
