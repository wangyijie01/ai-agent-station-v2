...
public class Step4LogExecutionSummaryNode extends AbstractExecuteSupport {
    ...
    protected String doApply(ExecuteCommandEntity requestParameter, DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext) throws Exception {
        if (dynamicContext.isWaitingUserInput()) {
            log.info("任务等待用户输入，已保存检查点，本轮传输结束");
    ...
    private void generateFinalReport(ExecuteCommandEntity requestParameter, DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext) {
        try {
            ...
            summaryPrompt = promptContextBuilder.appendExecutionContext(summaryPrompt, requestParameter);
            ...
            if (summaryResult == null || summaryResult.isBlank()) {
                throw new IllegalStateException("总结模型返回空结果");
            }
            ...
            agentRunService.checkpoint(requestParameter, "SUMMARY", dynamicContext.getStep(),
                    summaryResult, "完成");
            
        } catch (Exception e) {
            log.error("生成最终总结报告时出现异常: {}", e.getMessage(), e);
            throw new IllegalStateException("生成最终总结失败", e);
        }
    }
