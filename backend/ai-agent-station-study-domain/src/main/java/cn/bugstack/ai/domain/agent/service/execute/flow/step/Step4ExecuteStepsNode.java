...
public class Step4ExecuteStepsNode extends AbstractExecuteSupport {
    ...
    @Override
    public String doApply(ExecuteCommandEntity request, DefaultFlowAgentExecuteStrategyFactory.DynamicContext dynamicContext) throws Exception {
        ...
        try {
            ...
            if (aiAgentClientFlowConfigVO == null) {
                throw new IllegalStateException("缺少 EXECUTOR_CLIENT 流程配置");
            }
            ...
            if (errorStats != null && !errorStats.isEmpty()) {
                throw new IllegalStateException("Flow 存在失败步骤: " + errorStats.keySet());
            }
            ...
            agentRunService.checkpoint(request, "FLOW_COMPLETED", dynamicContext.getStep(),
                    dynamicContext.getExecutionHistory().toString(), "完成");
     ...
     * 按顺序执行规划步骤
     */
    private void executeStepsInOrder(ChatClient executorChatClient,
        ...
        for (Integer stepNumber : stepNumbers) {
            ...
            if (stepContent != null) {
                executeStep(executorChatClient, stepNumber, stepKey, stepContent, dynamicContext, command);
            } else {
     ...
     * 执行单个步骤
     */
    private void executeStep(ChatClient executorChatClient,
                             Integer stepNumber,
                             String stepKey,
                             String stepContent,
                             DefaultFlowAgentExecuteStrategyFactory.DynamicContext dynamicContext,
        ...
        try {
            ...
            String executionResult = executorChatClient.prompt()
                    .user(promptContextBuilder.appendExecutionContext(
                            buildStepExecutionPrompt(stepContent, dynamicContext), command))
                    .toolContext(toolContext(command))
                    .call()
                    .content();

            if (executionResult == null || executionResult.isBlank()) {
                throw new IllegalStateException("Flow 第 " + stepNumber + " 步返回空结果");
            }
            ...
            dynamicContext.getExecutionHistory().append("\n### ").append(stepKey).append('\n')
                    .append(executionResult).append('\n');
            agentRunService.checkpoint(command, "FLOW_EXECUTION", stepNumber,
                    executionResult, "执行下一步骤");
